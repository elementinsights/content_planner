...
export async function filterRelevantKeywords(
  ...
  for (let i = 0; i < records.length; i += BATCH) {
    ...
    try {
      ...
      for (const k of kept) if (typeof k === 'string') keepNorms.add(norm(k));
      batches++;
    } catch (err) {
      // Fail open to the lexical filter for just this batch — never lose keywords to an API blip.
      llmFailures++;
      for (const r of chunk) if (lexical.isRelevant(r.keyword)) keepNorms.add(norm(r.keyword));
  ...
  const kept = records.filter((r) => keepNorms.has(norm(r.keyword)));
